...
<!-- STATUS:BEGIN — generated by tools/readme.mjs, do not edit by hand -->
| | |
|---|---|
| Domain | Dirt to Dinner, extending into chemistry, metallurgy, biology, medicine and the built world |
| Elements | **8,567** (8,211 workshop · 356 folklore) |
| Recipes | **10,517** (9,537 merges · 980 verb processes) |
| Starters | Stone · Water · Sun · Seed |
| Reachable | **8,567 / 8,567** from the four starters |
| Sourced | **10,517 / 10,517** recipes, every URL machine-verified |
| Independently verified | **8,428 / 10,517** (80%) carry `verified: true` |
| Routes per element | **1.23** · 7,006 still have exactly one |
| Hazards | **343**, covering 622 elements |
| Scale span | **20 orders of magnitude**, 10^-10 m to 10^9 m |
| Drawings | **7,875** hand-drawn; 692 on a family fallback — a tracked backlog, not a blocker |
| Roles | **8,567 / 8,567** — what each noun is to a making |
| Sense-checked | **1,848** single-word names read; 181 on the watchlist |
| Places | **442 / 442** of a checklist that is part external, so it can disagree with us |
| Parts lists | **1,040** written |
| Effects on a body | **621 / 622** cautioned elements answered and **39 / 39** of the nutrients a body cannot make supplied; 79 rows are things the body is better off for, 5 elements are both, 5 carry a dose ladder; **549 / 549** have had the article they cite read and found to carry the claim |
...
while the corpus held 7,423 — and then did it again, claiming 7,576 against 8,567.*
<!-- STATUS:END -->
...
data/recipes.json    in[], verb?, at?, out, why, src, verified?
data/verbs.json      the eight process verbs and their unlock conditions
data/roles.json      what each noun IS to a making — material, technique,
                     process, tool, maker, site, form, occupant, subject,
                     evidence. Five contribute and five do not, which is what
                     lets a check ask whether a recipe says HOW a thing is made
data/homonyms.json   the sense each ambiguous word carries here, and the words
                     that would betray a different one
data/needs.json      what would actually go into one — a parts list, not a
                     recipe, so the gaps fall out mechanically
data/cautions.json   hazards, by severity: acute, contact, conditional, cumulative
data/places.json     landmarks, and a world checklist that is part external
data/art-pending.json  which elements are still on a family drawing
data/coverage-log.json a snapshot per run, so movement is visible
```

Five of those are **curated fields**: facts about a thing that no tag, no
recipe and no heuristic can supply. Four heuristics for them have been built,
measured and thrown away — one returned 684 candidates of which about two were
real, another 1,392, another 2,806. Curation beat inference every time, which
is why they are files a person writes rather than rules a tool applies.
...
**The audit is a standing process, not a finished one.** `tools/audit.mjs`
fetches every cited article — prose and infobox — and asks whether the numbers
in our sentence are actually in it. On 5 Sep it checked 321 claims and found 37
numbers the cited article does not contain, several of them heat bands added the
night before. Those are corrections owed, not errors caught and fixed.

It also found a bug in itself that had been there since the stage was written:
the infobox check called a function that does not exist anywhere in the file,
the ReferenceError went into a bare `catch {}`, and every article was reported
as having no infobox. Gold's melting point, butane's boiling point in kelvin and
every other figure that lives only in an infobox had been reported unsupported
for as long as the check had existed — while looking exactly like a check doing
its job.

The early hand audit, below, is what established the error rate that justifies
all of this:
...
1. **Coverage, not volume.** `node tools/coverage.mjs` reports what fraction of
   the corpus each mechanism has actually seen, because a green run on a sample
   looks identical to a green run on everything. Ten of twelve now cover it all;
   `needs` is at 21% and the source audit has a design ceiling of 39%, since it
   only examines recipes whose prose carries a number.
2. **The drawing backlog is deliberate.** 367 elements render from their family
   rather than a `def()` of their own, so that authoring never waits on
   artwork. `data/art-pending.json` is the list and it is a coverage row, not a
   footnote.
