...
const ROLES = read('data/roles.json');
...
const MECHANISMS = [
  ...
  { id: 'audit', file: 'tools/audit.mjs', part: 'gate',
    ...
    how: `Pulls every figure, date and named person out of the teaching text — and out of a recipe's temperature band, which is a numeric claim like any other — fetches the cited article, and looks for them. A claim that cannot be found in its own source is not automatically wrong, but it is unsupported, and it is listed. Where the prose does not carry a fact the article's infobox is pulled as a second chance, because melting points and dimensions and dates live there rather than in the text.`,
    found: `This is the mechanism the whole project rests on. Auditing our own confident, fluent, well-written prose against primary reporting found a first-pass error rate near half. A greenhouse does not work by trapping infrared; flint was given obsidian's record; film does not work by persistence of vision; Darwin coined "living fossil" about the platypus, not the coelacanth. All fluent. All wrong. <b>And the check itself was wrong six ways</b> — a comma after a year, a diacritic, a possessive, a chemical formula, an en dash, a stray quote mark — each of which made it report correct content as unsupported. Clearing those took the attribution count from 50 to 0 and made the remaining backlog worth reading. A check that is more than half noise is a check nobody finishes.`,
  ...
  { id: 'roles', file: 'tools/roles.mjs', part: 'truth',
    asks: 'What is this noun to the thing being made — the stuff, the method, or the reason?',
    how: `An element's tags say what it belongs to. They never say what part it plays when something is made from it. So <code>data/roles.json</code> gives each element one curated role: five that contribute to a making — material, technique, process, tool, maker — and five that do not — site, form, occupant, subject, evidence. With that field written down, "does this recipe say how the thing was made?" stops being a guess about tag vocabulary and becomes a lookup. An input with no role recorded is reported as unjudgeable rather than as a fault, because a partial file is only honest if the tools reading it say where it runs out.`,
    found: `Marble and Mausolus are both inputs to the Mausoleum at Halicarnassus. One is the stone it is built of; the other is the man it was built for, and Artemisia II is the one who built it. No tag in this corpus tells those three apart, which is why the tag-based version of this check flagged 65 of 334 places and, read through by hand, found nothing wrong with any of them. The role field found 62 — all of them real. Mount Fuji was made from <code>stratovolcano + japan</code> while its own prose said it "builds itself out of its own ash and lava"; Hōryū-ji came from <code>pagoda + dendrochronology</code>, which dated the temple rather than building it; Chartres from <code>cathedral + fire</code>; Nazca from <code>geoglyph + peru</code>. In almost every case the sentence was already right and the gesture underneath it named something else — the same fault class as the homonym list, one level up.`,
    now: `${n(Object.keys(ROLES.roles).length)} elements carry a role, covering every input to a place recipe.` },
